...
## Set up a database
To demonstrate the geospatial query possibilities of PostGREST, we to set up a
...
from scratch everytime but it is convenient for this example.
...
user account that has `select` privelges for the `plot` table.

```{.sql include="setup_db.sql" start-line=23 end-line=28}
```

### define `query_bbox` function
PostGREST opens up a powerful querying API out-of-the-box, but if you want to
use any of the PostGIS functions you need to create use user-defined functions.
...
the bounding box.
When you send a request to this endpoint, the records (`setof api.plot`) will be
returned as JSON:
...
### query using `curl`
The [PostGREST API docs](https://postgrest.org/en/stable/api.html) has an
extensive guide to interacting with the API, so definitely check that out.
...
### query using Python
We can use the `requests` library to send queries to PostgREST, too.
